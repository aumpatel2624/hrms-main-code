...
The actual per-employee, per-leave-type grant of N days.

![Leave Allocations](../screenshots/leave-allocation-light.png "light")

![Leave Allocations](../screenshots/leave-allocation-dark.png "dark")

## When you would use this

Usually created automatically by a Leave Policy Assignment's Grant Allocations action — create one by hand only for a one-off manual grant.
...
- **New Leaves Allocated** *(a number)* — Only used on create — use Adjust below to change it afterward.
- **Unused Leaves (carried forward)** *(a number)*
- **Add unused leaves from previous allocations** *(a yes/no tick box)*
- **Leave Period** *(chosen from a list)*
- **Leave Policy** *(chosen from a list)*
...
## Adding a Leave Allocation

Press **Add Leave Allocation** at the top right of the list. That opens a blank form.

1. Fill in the fields described above. Required ones are marked with an asterisk.
2. Press **Create leave allocation** at the bottom of the form.
...
![Adding a Leave Allocation](../screenshots/leave-allocation-add-light.png "light")

![Adding a Leave Allocation](../screenshots/leave-allocation-add-dark.png "dark")

*Needs the **write** permission — without it the button is not shown.*

## Viewing a Leave Allocation
...
![Viewing a Leave Allocation](../screenshots/leave-allocation-view-light.png "light")

![Viewing a Leave Allocation](../screenshots/leave-allocation-view-dark.png "dark")

*Needs the **read** permission — without it the button is not shown.*

## Editing a Leave Allocation
...
![Editing a Leave Allocation](../screenshots/leave-allocation-edit-light.png "light")

![Editing a Leave Allocation](../screenshots/leave-allocation-edit-dark.png "dark")

*Needs the **edit** permission — without it the button is not shown.*

## Deleting a Leave Allocation

Press the delete icon on a row. You are asked to confirm first, and nothing is removed until you do.

If something else in the system still refers to this leave allocation, the deletion is refused and you are told what is using it. Clear or reassign those first, then try again.
...
> HR User and HR Manager only — no Employee access yet (own-scoping is a second-fork follow-up).
