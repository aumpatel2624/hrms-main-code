 ...
 * /email-for/triggers:
 *   get:
 *     summary: Registry of email triggers (ADR-015) — which are already assigned to an Email For
 *     tags: [Email For]
 *     security:
 *       - bearerAuth: []
 *     responses:
 *       200:
 *         description: The trigger registry, each entry flagged with the Email For that claims it (if any)
 *         content:
 *           application/json:
 *             schema:
 *               $ref: '#/components/schemas/SuccessResponse'
 */
// Registered before /:emailForId so "triggers" is never read as an id.
router.get(
  "/email-for/triggers",
  authMiddleware(ANY_ROLE),
 ...
 */
// SECURITY: Auth + secure upload with validation
router.post(
  "/email-templates/upload-signature",
  authMiddleware(ANY_ROLE),
  checkPermission("/email-template", "edit"),
