...
### Payroll — Structure & Assignment (module 10, complete 2026-09-11 — ADR-026, `feat/payroll-structure`)

`SalaryComponent` — master line item (Basic Salary, HRA, PF, TDS, ...): `salaryComponentName`, `abbreviation` (auto-derived from initials if left blank, e.g. "Basic Salary" -> "BS", de-duplicated with a `_N` suffix scoped to `companyId`), `type` (Earning/Deduction/Employer Contribution), the flag booleans (`isTaxApplicable`, `dependsOnPaymentDays`, `doNotIncludeInTotal`, `statisticalComponent`, `roundToNearestInteger`, `exemptedFromIncomeTax`, `removeIfZeroValued`, `variableBasedOnTaxableSalary`, `arrearComponent`, `accrualComponent`), `companyId`. `arrearComponent` and `variableBasedOnTaxableSalary` are mutually exclusive (400 if both set); `accrualComponent` is only valid when `type === "Earning"` (400 otherwise). No GL fields (`accounts`) and no Benefits fields (`isFlexibleBenefit`/`maxBenefitAmount`/`payoutMethod`) — both dropped per ADR-016/ADR-026.

`SalaryStructure` — a pay template: `companyId`, `payrollFrequency` (Monthly/Fortnightly/Bimonthly/Weekly/Daily, server-required), `isActive`, optional `leaveEncashmentAmountPerDay` (Q-14), `currency`, and three embedded `SalaryDetail` arrays (`earnings`/`deductions`/`employerContributions`). Each `SalaryDetail` row carries `salaryComponentId`, a ONE-TIME denormalized copy of that component's flags taken at row-creation time (**not a live join** — editing the master `SalaryComponent` later never retroactively changes an existing structure's row; matched by the row's own Mongoose subdocument `_id` across an update, so only a genuinely new row gets a fresh copy), plus `condition`/`formula`/`amountBasedOnFormula`/`amount`/`defaultAmount` (the last server-computed). `totalEarning`/`totalDeduction`/`netPay` are recomputed server-side on every save (`utils/payrollCtc.js`'s `computeStructureTotals`) — `totalEarning` excludes rows flagged `statisticalComponent`/`doNotIncludeInTotal`, `totalDeduction` is an unfiltered sum, `netPay = totalEarning - totalDeduction`.

`SalaryStructureAssignment` — assigns one `SalaryStructure` to one employee from a `fromDate` (no `toDate` field at all — an assignment is open-ended until superseded by a later `fromDate`). `employeeId`/`salaryStructureId`/`fromDate`/`companyId` (fetched from employee)/optional `base`/`variable`/`currency` (fetched from structure)/`leaveEncashmentAmountPerDay` (fetched from structure unless explicitly supplied). Validates: active-employee guard, `fromDate` within `[dateOfJoining, relievingDate]`, structure's company matches the employee's, and the ONLY uniqueness invariant — exact-duplicate `fromDate` for the same employee (a real unique index, not just a `findOne` check). `annualGrossEarning`/`ctc` are server-computed (`utils/payrollCtc.js`'s `computeCtcAndGross`) by evaluating the structure's rows with real `base`/`variable` in context: `grossPerPeriod` (earnings, same exclusion rule as `totalEarning`) × `PERIODS_PER_YEAR[payrollFrequency]` for `annualGrossEarning`; `(grossPerPeriod + employerContributionsTotal) × periods` for `ctc`.

**The formula/condition evaluator** (`utils/payrollFormula.js`) — a hand-written recursive-descent parser/evaluator (no `eval`/`Function`) for `SalaryDetail.formula`/`.condition` text: arithmetic (`+ - * /`, unary minus, parentheses), comparison (`== != < > <= >=`), boolean (`and`/`or`/`not`), numeric literals, variable references (case-sensitive, resolved against a component-abbreviation context plus `base`/`variable`), and exactly five functions (`round`/`min`/`max`/`ceil`/`floor`). An unknown identifier or function name raises a clear `Error`. `evaluateComponentTable` (`utils/payrollCtc.js`) walks a row table in order, mutating a shared context so a later row/table can reference an earlier row's resolved value by abbreviation — a `condition`-gated row that evaluates false is skipped entirely, contributing 0 to any total and injecting NO context entry at all (matching source's real behavior exactly).

`getCurrentSalaryStructureAssignment(employeeId, asOfDate)` (`utils/payrollAssignment.js`) — resolves "which structure applies to this employee on this date" by query, not a stored flag: the latest assignment (by `fromDate`) with `fromDate <= asOfDate`, **no company filter**. Used by the Q-14 Leave Encashment retrofit and anywhere else "what's this employee's current structure" is needed.

**Bulk Salary Structure Assignment** — stateless bulk-action endpoints, no stored model (same shape as Leave Control Panel/Shift Assignment Tool): `eligible-employees` (active employees joined-by/not-yet-relieved-as-of a `fromDate`, matching optional company/department/grade/employment-type filters, excluding anyone who already has an assignment for that exact `fromDate`) and `assign` (`employeeIds[]`/`salaryStructureId`/`fromDate`/`base`/`variable`, one `SalaryStructureAssignment` per employee, per-item try/catch isolation, always synchronous). Both this and the single-assignment endpoint share one `createSalaryStructureAssignmentCore` function.

`EmployeeGrade` gained `defaultSalaryStructureId`/`defaultBasePay` (both optional) — the retrofit its own module-2 code comment had already flagged as pending this module.

### Payroll — Run (module 11 complete, ADR-027)

`PayrollPeriod` — company-scoped date-range bookkeeping: `companyId`/`startDate`/`endDate`/`isActive`. Rejects an overlapping range for the **same** company only; two different companies' periods may freely overlap. No `Payroll Period Date` child table (a confirmed-dead field in source itself, not ported even as inert schema) and no tax-annualization role (that needs `Income Tax Slab`, `OPEN-QUESTIONS.md` Q-21).

`PayrollSettings` — a true global singleton (`SeoSettings.js`'s exact `key: "default"` + unique-index pattern, no Company dimension at all): `payrollBasedOn` (Attendance/Leave Application), `considerUnmarkedAttendanceAs` (Present/Absent), `includeHolidaysInTotalWorkingDays`, `considerMarkedAttendanceOnHolidays`, `dailyWagesFractionForHalfDay` (default 0.5), `disableRoundedTotal`, `showLeaveBalancesInSalarySlip`. `getPayrollSettingsDoc()` finds the one row or creates it with defaults on first read.

`SalarySlip` — one payslip for one employee for one period; **a point-in-time snapshot** (INV-41), never recomputed after creation. `employeeId`/`salaryStructureAssignmentId` (resolved via `getCurrentSalaryStructureAssignment` as of `endDate` when not supplied explicitly)/`companyId` (from the employee)/`startDate`/`endDate` (exact-duplicate `(employeeId, startDate, endDate)` rejected, a real unique index). Payment-days snapshot: `workingDays`/`totalWorkingDays`/`paymentDays`/`lwpDays`/`absentDays`/`halfDayDays` (see CALC-4, `RULES.md`). Embedded `earnings`/`deductions`/`employerContributions` reuse `SalaryStructure.js`'s exported `SalaryDetailSchema` unchanged, copied from the resolved assignment's structure and scaled per CALC-4. Server-computed `grossPay`/`totalDeduction`/`netPay`. Embedded read-only `leaves[]` — a scoped-down 3-field snapshot per Leave Type covering the employee as of `endDate` (`leaveTypeId`/`allocated`/`used`/`available`, not source's full 5-field shape — see `DECISIONS.md` ADR-027 "As built"), populated only when `PayrollSettings.showLeaveBalancesInSalarySlip` is on. `status`: `draft`/`submitted`/`withheld`/`cancelled` (FLOW-4). Overlaid with dynamic `displayStatus: "withheld"` and `isSalaryWithheld: true` when covered by an active `SalaryWithholding`.

`PayrollEntry` — company-scoped bulk payroll run (`companyId`, `startDate`, `endDate`, `payrollFrequency`, `validateAttendance`, `employeeDetails[]`, `status`: draft/submitted/cancelled). Embedded `employeeDetails` tracks per-employee outcomes (`employeeId`, `employeeName`, `departmentId`, `designationId`, `salarySlipId`, `isSalaryWithheld`, `status`: pending/created/failed/submitted, `failureReason`). Eligibility selection validates matching active structure assignment and excludes employees with pre-existing slips for the period or missing attendance when `validateAttendance` is enabled. Supports bulk `create-slips` and `submit-slips` with individual try/catch isolation (INV-43), and cascading `cancel`.

`SalaryWithholding` — frequency-aligned schedule of withheld salary cycles (`employeeId`, `companyId`, `fromDate`, `payrollFrequency`, `numberOfWithholdingCycles`, `cycles[]`, `status`: draft/withheld/released/cancelled). Cycles are generated by strictly walking one frequency-period at a time (`addPayrollPeriods`) with month-end clamping (INV-44). Vacuously derives status "released" when the cycle list is empty. Manual per-cycle (`release-cycle`) and bulk (`release-all`) release actions record release timestamps and optional reference text without creating General Ledger postings.
...
### Payroll — Benefits (module 13 complete, ADR-029)

`EmployeeBenefitApplication` — employee flexible benefit packaging plan: `employeeId`, `companyId`, `payrollPeriodId`, `date`, embedded `employeeBenefits[]` (`salaryComponentId`, `amount`, `maxAmount`), `maxBenefits` (overall cap from assignment/structure), `totalAmount` (server-computed sum of detail rows), `remainingBenefit` (`maxBenefits - totalAmount`), `status` (`draft`/`submitted`/`cancelled`). Detail rows must reference active flexible benefit components (`isFlexibleBenefit: true`, Earning only) and respect individual component caps (`amount <= maxBenefitAmount`) and the overall ceiling (`totalAmount <= maxBenefits`). When `PayrollSettings.mandatoryBenefitApplication` is false, `SalaryStructureAssignment.employeeBenefits` serves as the fallback source for slip benefit accruals.

`EmployeeBenefitClaim` — reimbursement claim against accrued benefit ledger: `employeeId`, `companyId`, `claimDate`, `salaryComponentId` (must be `isFlexibleBenefit: true` and `payoutMethod: "Against Claim"` or `"Accrued On Date Pro-rata"`), `claimedAmount` (> 0), `yearlyBenefit`, `maxAmountEligible` (read-only balance snapshot), `additionalSalaryId`, `status` (`draft`/`submitted`/`cancelled`). Submit verifies cumulative claims do not exceed annual eligibility / accrued ledger balance, creates an active `AdditionalSalary` row (`type: "Earning"`, `refDoctype: "EmployeeBenefitClaim"`), and on slip submission writes a deduction/debit row to `EmployeeBenefitLedger`. Cancel cascades to cancel the linked `AdditionalSalary` and soft-deletes linked ledger entries.

`EmployeeBenefitLedger` — append-only domain ledger tracking flexible benefit debits, credits, and accruals (similar to `LeaveLedgerEntry`, no General Ledger postings): `employeeId`, `companyId`, `salaryComponentId`, `payrollPeriodId`, `date`, `amount` (positive for accruals/credits, negative for claim payouts/debits), `refDoctype` (`SalarySlip`, `EmployeeBenefitClaim`, `PayrollCorrection`, etc.), `refDocnameId`, and soft-delete fields. Read-only public API (`GET /` and `POST /search` and `GET /:id`; direct creation/update/deletion returns 404).

`PayrollCorrection` — retroactive adjustment worksheet for attendance/LWP reversals: `employeeId`, `companyId`, `salarySlipId`, `payrollPeriodId`, `payrollDate`, `daysToReverse`, `status` (`draft`/`submitted`/`cancelled`), embedded `earningArrears[]`, `deductionArrears[]`, and `accrualArrears[]`. Calculated via `calculatePayrollCorrectionBreakup` by re-evaluating historical submitted salary slips with revised payment days/LWP days. Submit generates active `AdditionalSalary` rows (`refDoctype: "PayrollCorrection"`) for non-zero earning/deduction arrears and inserts `EmployeeBenefitLedger` entries for accrual arrears. Cancel cascades to cancel generated `AdditionalSalary` rows and soft-deletes linked `EmployeeBenefitLedger` entries.
