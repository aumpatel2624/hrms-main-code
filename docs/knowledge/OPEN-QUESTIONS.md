...
| Q-9 | `Job Applicant.source == "Employee Referral"` status-sync (write back onto `EmployeeReferral` when its linked Job Applicant's status changes) and Interview Feedback's per-skill `Skill Assessment` ratings (ADR-019) are both unbuilt. `EmployeeReferral` itself now exists (module 5, ADR-021, with a working `createJobApplicant` action) — the *reverse* sync (Job Applicant status changes flowing back onto the originating Employee Referral) is still not built. `Skill` still doesn't exist (module 6). | Module 6 for Skill Assessment; a Recruitment follow-up touch for the reverse referral sync, whenever it's prioritized | 2026-09-10 | |
| Q-10 | This project's dev database has zero `Country`/`State`/`City` rows and zero generic `User` (the starter's pre-HRMS login-account model, `User.countryId`/`stateId`/`cityId` all required) — found while verifying Recruitment's role matrix, which needed a throwaway `User` to test as HR User/Employee-role. Nothing HRMS-specific seeds these yet. Does Apidel need real geography master data seeded, or should `User`'s required country/state/city be reconsidered once `Employee.userId` self-service provisioning (deferred, ADR-018) actually gets built — a real login account might not need full postal geography for every HR-created Employee login. | Whichever module first provisions a real self-service `Employee` login | 2026-09-10 | |
