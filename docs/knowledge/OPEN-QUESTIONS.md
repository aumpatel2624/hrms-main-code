...
| Q-8 | `Job Opening`/`Job Offer` vacancy checks against `Staffing Plan` (ADR-019) are unbuilt — `Staffing Plan`/`Staffing Plan Detail` don't exist until module 5 (Employee Career Events). Once they do, should Recruitment's validation be retrofitted to enforce them, or is unlimited-vacancy hiring acceptable going forward? | Module 5, when it's built — and Recruitment needs a follow-up touch after | 2026-09-10 | |
| Q-9 | `Job Applicant.source == "Employee Referral"` status-sync and Interview Feedback's per-skill `Skill Assessment` ratings (ADR-019) are both unbuilt — waiting on `Employee Referral` and `Skill` (modules 5 and 6). Same shape as Q-8: a follow-up touch to Recruitment once those modules exist, not forgotten. | Modules 5 and 6, plus a Recruitment follow-up after each | 2026-09-10 | |
