...
| INV-8b | A role below System Manager never sees or writes a row belonging to a different Company — the full per-request company-confinement mechanism (query-level, not just uniqueness). | **Not yet built.** Every Organization Setup screen is reachable only by HR User/HR Manager (INV-8c) and there is one seeded Company today, so this hasn't bitten yet. Mechanism decided in whichever module first needs it (`OPEN-QUESTIONS.md` Q-4) — likely Employee Records (module 2). |
